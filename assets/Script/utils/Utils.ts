 ...
 */
  ...
  public static showUI(node: cc.Node, audio: cc.AudioClip, delay: number, canPlayMusic: boolean, StartScale?: number, EndScale?: number, callback?: Function, time1?: number, time2?: number) {
    setTimeout(() => {
      ...
      cc.tween(node)
        .to(time1, { scale: StartScale }, { easing: 'sineInOut' })
        .to(time2, { scale: EndScale }, { easing: 'sineInOut' })
        .call(() => {
  ...
  * @param times 播放次数
  */
  public static SetScale(node: cc.Node, scale: number, time: number, isRepeat?: boolean, times?: number, cb: Function = null) {
    ...
    if (isRepeat) {
      ...
      if (time == null) time = 1
      cc.tween(node).repeat(times, tween)
        .call(() => {
          cb && cb()
        })
        .start()
  ...
  public static closeUI(node: cc.Node, cb: Function = null, time: number = 0.4) {
    cc.tween(node)
      .to(time, { scale: 0 })
      .call(() => {
        cb && cb()
  ...
  public static openUI(node: cc.Node, cb: Function = null, time: number = 0.3, delay: number = 0) {
  ...
  /**
 * 旋转出场
 *@param rotateNum 旋转周数
 */
  public static rotateShowUI(node: cc.Node, audio: cc.AudioClip, startScale: number, EndScale: number, actionTime: number, rotateNum: number, callback?: Function) {
    node.scale = startScale
    node.active = true
    cc.audioEngine.play(audio, false, 1)
    let rotateAction = cc.rotateTo(actionTime, 360 * rotateNum)
    let scaleAction = cc.scaleTo(actionTime, EndScale)
    let action = cc.spawn(rotateAction, scaleAction)
  ...
  * 批量置灰接口
  */
  public static setGrey(node: cc.Node | cc.Component, grey: boolean = true) {
    const components = node.getComponentsInChildren(cc.RenderComponent)
    if (components.length > 0) {
      ...
      var material = cc.Material['getBuiltinMaterial'](grey ? cc.Material.BUILTIN_NAME.GRAY_SPRITE : cc.Material.BUILTIN_NAME.SPRITE)
  ...
  /**手指移动 */
  public static fingerMove(node, posArr) {
    const index = posArr.findIndex(pos => pos.equals(node.position));
    if (index !== -1) {
      posArr.splice(index, 1);
    }
    if (posArr.length > 0) {
      const random = this.getRandomInt(0, posArr.length - 1);
      node.position = posArr[random];
    } else {
      console.log('boxPosArr 数组为空，无法设置手指位置');
    }
  }
}